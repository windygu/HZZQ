  ...
  // alert(rootPath);
   ...
   if (window.document.all)//IE判断window.showModalDialog!=null
   {
      var returnvalue;
       ...
       {
           returnvalue = window.showModalDialog(rootPath + "Common/Select.aspx?TableName=" + strTableName + "&ReturnField=" + strReturnField + "&filterField=" + strFilterField + "&filterValue=" + strFilterValue + "&Option=1", "",
                                         "top=0;left=0;toolbar=no;menubar=no;scrollbars=no;resizable=no;location=no;status=no;dialogWidth=700px;dialogHeight=460px");
       }
       else
       {
        returnvalue=window.showModalDialog(rootPath+"Common/Select.aspx?TableName="+strTableName+"&ReturnField="+strReturnField+"&Option=1","",
                                         "top=0;left=0;toolbar=no;menubar=no;scrollbars=no;resizable=no;location=no;status=no;dialogWidth=700px;dialogHeight=460px");
       }
       if(returnvalue==null)
       {
   ...
   else
   {
        ...
        //打开窗口
        var url=rootPath+"Common/SelectDialog.aspx?TableName="+strTableName+"&ReturnField="+strReturnField+"&time="+time+"&targetControls="+strTarget;
  ...
  // alert(rootPath);
   if (window.document.all)//IE判断window.showModalDialog!=null
   {
      var returnvalue;
       if(strFilterField!=null && strFilterField.length>0)
       {
         returnvalue=window.showModalDialog(rootPath+"Common/SelectDialog.aspx?TableName="+strTableName+"&ReturnField="+strReturnField+"&filterField="+strFilterField+"&filterValue="+strFilterValue+"&sqlFilter="+sqlFilter+"&time="+time,"",
                                         "top=0;left=0;toolbar=no;menubar=no;scrollbars=no;resizable=no;location=no;status=no;dialogWidth=700px;dialogHeight=460px");
       }
       else
       {
        returnvalue=window.showModalDialog(rootPath+"Common/SelectDialog.aspx?TableName="+strTableName+"&ReturnField="+strReturnField+"&sqlFilter="+sqlFilter+"&time="+time,"",
                                         "top=0;left=0;toolbar=no;menubar=no;scrollbars=no;resizable=no;location=no;status=no;dialogWidth=700px;dialogHeight=460px");
       }
       if(returnvalue==null)
       {
