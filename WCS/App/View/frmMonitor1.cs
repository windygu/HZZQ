    ...
    public partial class frmMonitor1 : BaseForm
    {
        ...
        void Monitor_OnCrane(CraneEventArgs args)
        {
            ...
            else
            {
                ...
                //    if (txt != null)
                //    {
                //        txt.Text = crane.WalkCodeg.ToString();
        ...
        private void tmCraneWorker1(object sender, System.Timers.ElapsedEventArgs e)
        {
            try
            {
                ...
                if (crane.Column == 100)
                    crane.Column = 0;
                crane.WalkCode = int.Parse(workmode.ToString());
        ...
        private void btnChangeMode_Click(object sender, EventArgs e)
        {
            ...
            //f.ShowDialog();
            //DataTable dt = bll.FillDataTable("WCS.SelectTask", new DataParameter[] { new DataParameter("{0}", "(WCS_TASK.TaskType='12' and WCS_TASK.State in('0','1'))") });
            
            //if (dt.Rows.Count > 0 && Program.mainForm.WorkMode>1)
            //{
            //    MessageBox.Show("还有未完成的任务，不可切换工作模式！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
            //    return;
            //}
            try
            {
                ...
                if (f.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    GetWorkMode();
                    //写入PLC
                    Context.ProcessDispatcher.WriteToService("ConveyorPLC2", "WorkMode", Program.mainForm.WorkMode);
                }
            }
            catch (Exception ex)
            {
        ...
        private void toolStripMenuItem_DelCraneTask_Click(object sender, EventArgs e)
        {
            string serviceName = "CranePLC1";
            int[] craneInfo = new int[6];
            ...
            if (!Program.mainForm.Run || (craneInfo[4] > 0 && craneInfo[4] != 5 && craneInfo[4] != 15 && craneInfo[4] != 6 && craneInfo[4] != 16))
            {
                if (DialogResult.Yes == MessageBox.Show("您确定要删除当前堆垛机任务吗？", "询问", MessageBoxButtons.YesNo, MessageBoxIcon.Question))
                {
                    ...
                    Logger.Info("堆垛机任务已删除");
                }
            }
            else
                MessageBox.Show("堆垛机无故障或故障信息不符合删除条件？", "询问", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
