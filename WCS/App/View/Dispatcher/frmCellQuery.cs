        ...
        private void pnlChart_Paint(object sender, PaintEventArgs e)
        {
            try
            {
                if (needDraw)
                {
                    ...
                    float adjustHeight = Math.Abs(size.Height - cellHeight) / 2-2;
        ...
        private void DrawShelf(DataRow[] cellRows, Graphics g, int top, Font font, float adjustWidth)
        {
            ...
            for (int j = 1; j <= columns; j++)
            {
                if (j == 1 && cellRows.Length < columns * rows)
                    continue;
                g.DrawString(Convert.ToString(j), new Font("微软雅黑", 12), Brushes.DarkCyan, left + (j - 1) * cellWidth + adjustWidth, top + cellHeight * (rows + 1) + 3);
            }
        }
